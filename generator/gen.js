...
const container = document.querySelector('.paragraph-display');
const input = document.querySelector('.text-input');
const button = document.querySelector('.submit');

button.addEventListener('click', generateP)

let message;
function generateP(){
    const Ivalue = input.value;
    if (isNaN(Ivalue) || Ivalue <= 0 || Ivalue > text.length){
        let random = Math.floor(Math.random() * text.length);
        message = `<p class="result">${text[random]}</p>`;
    }else{
        let newText = text.slice(0, Ivalue);
