...
const displayText = document.querySelector('.lorem-text');
...
form.addEventListener('submit', (e)=>{
    ...
    if (isNaN(inputValue)||inputValue < 0||inputValue > 9) {
        displayText.innerHTML = `<p class="result">${text[random]}</p>`;
    }else{
        let tempText = text.slice(0,inputValue)
        tempText = tempText.map(content => {
            return `<p class="result">${content}</p>`;
        }).join("");
        displayText.innerHTML = tempText;
    }
})